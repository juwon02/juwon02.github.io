---
layout: post
title: "pygame으로 게임을 만들어보자-4. GAME OVER 사운드를 넣어보자!"
excerpt: "pygame 그게 무엇인가"
writer: Juwon02
categories: [pygame]
tags: [pygame]
---
# 🛠️GAME OVER 사운드를 넣어보자
생명이 다 닳고 끝나면 꼭 필요한게 있다. 바로 GAME OVER사운드 인것이다. GAMEOVER 사운드는 플레이어을 도발(?) 약간의 끈기를 넣어주는 역할을 하기도 하고 그냥 끝나버리면 밋밋하니 넣으면 좋을 것 같아서 넣어보기로 했다.

이번 프로제크에서 팀원이 무료BGM 사이트를 공유해줘서 나도 공유해볼생각이다 <https://opengameart.org/>여기 생각보다 유용한 BGM이 많으니 필요하신 분들은 여기를 사용하면 좋을것 같다.

🔶배경음악 초기화 및 설정 추가
pygame.mixer.init()  오디오 장치 초기화 역할 컴퓨터의 오디오 하드웨어를 준비하여 음악 및 효과음을 재생할 수 있도록 설정한다.
pygame.mixer.music.load() 음악을 로드하여 pygame.mixer.music.play() 음악을 실행한다

❗여기서 소리가 잘 안들리는 경우가 있다. 이때 팀원이 찾아준게 pygame.mixer.music.set_volume()이것이다! 현재 음악의 볼륨을 1로 했을 때 불륨을 높이려면 1보다 크게, 낮추려면 1보다 작게 설정하면 된다.

### RETRY 화면에도 BGM을 추가하자
RETRY화면에 음악이 없으면 섭섭하니 여기도 음악을 추가해보자
